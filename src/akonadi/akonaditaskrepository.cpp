...
KJob *TaskRepository::update(Domain::Task::Ptr task)
{
    if (task->recurrence() && task->status() == Domain::Task::Complete) {
        ...
        *ocurrence = *task;
        ocurrence->setRecurrence(Domain::Recurrence::Ptr());
        ocurrence->setStatus(Domain::Task::Complete);
        ocurrence->setProperty("relatedUid", task->property("todoUid"));

        if (!task->startDate().isValid()) {
            qWarning() << "A recurring todo must always have a valid start date";
            task->setStartDate(task->dueDate().isValid()?task->dueDate():QDateTime::currentDateTime());
        }
        ...
        QDateTime nextStartDate = todo->recurrence()->getNextDateTime(KDateTime(task->startDate())).dateTime();
